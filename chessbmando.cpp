...
private:
    ...
    bool AreSquaresLegal(int iSrcRow, int iSrcCol, int iDestRow, int iDestCol, GamePiece* GameBoard[8][8]) {
// square is unoccupied or occupied by opposite color
        if ((iSrcCol == iDestCol + 1) || (iSrcCol == iDestCol - 1)) {
...
public:
    ...
    void GetNextMove(GamePiece* GameBoard[8][8]) {
        ...
        do {
        	cout<<" wanna make a move mando 2nd row 3rd column (type=23) to 3rd row 3rd column(type=33)"<<endl<<endl;
...
// check that the source and destination are different
    ...
    bool IsGameOver() {
// If current player is not moving, we have a stalemate or checkmate
        bool bCanMove(false);
